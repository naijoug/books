...
| 9 | 部署与监控 | 部署形态、可观测性、告警止损、弹性、只读灰度 / 人工审批灰度、回滚和发布安全门禁 |
...
3. 接着读第 9 章，准备部署形态、监控指标、告警、只读灰度 / 人工审批灰度与回滚，并把第 8 章评估报告中的失败样本、脱敏执行 trace 和审计事件接入发布报告；发布报告里的灰度范围、禁用工具、回滚开关和 `safe_trace_links` 要能直接回填附录 A/B。

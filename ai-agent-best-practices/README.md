...
10 章正文和 3 份生产就绪附录已进入书稿主线，继续进行事实核验、示例验证和交叉引用整理。读者可按下表选择章节；局部修订只需读取目标段落及直接关联材料。后半部分的发布审查流程用于读者的 Agent 上线场景，不是每次编辑本书的前置要求。
...
| 9 | 部署与监控 | 部署形态、可观测性、告警止损、事故回放演练、弹性、只读灰度 / 人工审批灰度、回滚、发布安全门禁、readiness 字段契约、无人值守任务交接日志和会后交接字段 |
...
| 附录 C | 上线评审会主持人脚本 | 把附录 A/B 和第 9 章发布报告转成 30～60 分钟评审会流程，帮助主持人收束证据、范围、结论、会后交接和下一步行动 |
...
3. 接着读第 9 章，准备部署形态、监控指标、告警、事故回放、只读灰度 / 人工审批灰度与回滚，并把第 8 章评估报告中的失败样本、脱敏执行 trace 和审计事件接入发布报告；线上异常恢复后，要能把脱敏回放样本反向补进第 8 章评估集和第 10 章安全门禁；发布报告里的灰度范围、禁用工具、回滚开关、`safe_trace_links` 和 `next_review` 要能直接回填附录 A/B/C。
4. 最后读第 10 章，把权限、数据、审批、审计日志、事故熔断 runbook 和伦理边界固化为上线门禁，让安全门禁报告可以直接引用第 8 章产出的评估证据。

## 90 分钟上线前审查工作流

如果读者已经有一个可运行的 Agent 原型，不必从头重读全书。可以把第 7-10 章压缩成一次 90 分钟审查，用来判断“今天能不能灰度”。执行前先统一五组术语，避免不同团队在评审会上把同一件事叫成不同名字：

| 术语 | 本书统一含义 | 不推荐写法 |
|---|---|---|
| 工具网关 | 服务端执行工具鉴权、参数校验、审批绑定、审计和熔断的确定性边界 | “权限网关”单独出现，容易让读者误以为只有鉴权没有运行期控制 |
| `safe_trace` / `safe_trace_links` | 已脱敏、可长期进入评审材料和调试报告的执行 trace 及其链接字段 | 把原始 trace 直接贴进门禁报告 |
| `restricted_trace` | 仅在事故或安全复盘中短期审批访问、带访问日志的受限 trace | “内部可见 trace”，因为内部可见不等于已脱敏 |
| 只读灰度 / 人工审批灰度 | 前者只允许读与草稿能力；后者允许部分写工具在服务端审批后执行 | “小流量灰度”，因为流量小不代表副作用小 |
| `warn` / `block` | `warn` 表示硬边界存在但范围受限；`block` 表示缺少权限、脱敏、审计、回滚等硬边界 | 把缺失硬边界写成“后续优化” |

第 10 章的工具网关段落会把这张术语表反向落到服务端控制点：读到“权限网关”时，应理解为工具网关中的权限策略能力；读到 `warn` / `block` 时，应回到硬边界是否存在，而不是看团队主观信心。这样根 README、正文第 10 章和附录 A/B/C 使用同一套评审语言。
...
| 会议结束后谁接手？ | 第 9 章 `9.7.4` 发布报告模板和第 10 章收口段 | 附录 C 的字段映射与会后交接样例 | 下一步命令、复审触发条件、交接 owner |

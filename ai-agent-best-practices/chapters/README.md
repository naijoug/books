...
| 8 | `08-testing-debugging.md` | 测试分层、离线评估、权限测试、执行链路调试和安全回归集 |
...
- 第 8 章：把关键任务写成 Golden Tasks，覆盖成功路径、失败路径、权限测试、敏感字段扫描和可进入发布门禁的安全回归集。
- 第 9 章：建立部署、监控、告警、灰度、回滚和成本止损机制，并把普通回归与安全硬门禁接入同一条发布流水线。
- 第 10 章：补齐 Prompt 注入防御、数据脱敏、人工审批、脱敏执行 trace、审计日志、事故熔断 runbook 与发布前安全检查清单。

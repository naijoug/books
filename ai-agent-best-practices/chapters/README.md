...
进入第 7-10 章和附录 A/B/C 前，先沿用根目录 `README.md` 的统一术语口径：服务端确定性边界统一叫“工具网关”；长期评审材料只引用 `safe_trace_links`；`restricted_trace` 只做短期审批访问；只读灰度不开放写副作用；人工审批灰度要求审批绑定工具名、参数快照、`args_hash`、资源范围、幂等键和过期时间；`warn` 只能用于硬边界已存在但范围受限的情况，缺少权限、脱敏、审计或回滚硬边界时必须写成 `block`。

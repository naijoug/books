...
如果时间有限，可以把这条路径压缩成一次 90 分钟上线前审查：先列工具风险表，再跑最小 Golden Tasks，然后补发布报告和安全门禁结论，最后只在证据链完整时开放灰度。根目录 `README.md` 中的“90 分钟上线前审查工作流”给出了时间盒和最小产物；附录 A 可作为空白核对表，附录 B 可作为填完后的门禁报告参考，并用反例提醒团队不要把缺失硬边界的 `block` 写成“后续优化”的 `warn`；附录 C 则提供主持人脚本和会后交接填写样例，帮助发布 owner 在 30～60 分钟会议里把讨论收束为允许范围、禁用范围、证据缺口、下一次复审条件和下一步可执行命令。

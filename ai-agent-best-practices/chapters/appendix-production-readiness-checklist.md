...
评审材料越多，术语越容易漂移。正式填写本清单前，先把下面口径写进会议说明或发布 issue：

| 术语 | 填表时的判断标准 |
|---|---|
| 工具网关 | 指服务端对工具调用做身份、租户、allowlist / denylist、参数范围、审批绑定、审计和熔断控制的统一边界；不要只写“Prompt 限制”或“前端确认”。 |
| `safe_trace` | 已脱敏、可进入长期评审材料的 trace。清单和门禁报告只保存 `safe_trace_links`，不复制用户原文、完整内部 URL、密钥或支付信息。 |
| `restricted_trace` | 仅用于事故调查或安全复盘的受限 trace，需要短期审批、访问日志和到期清理；它不能替代 `safe_trace` 进入常规发布报告。 |
| 只读灰度 | 只允许读取、检索、分类、草稿生成等无外部副作用能力；任何自动发送、退款、改合同、改 SLA 都不属于只读。 |
| 人工审批灰度 | 允许部分写工具执行，但审批必须在服务端绑定工具名、参数快照、资源范围、`args_hash`、`idempotency_key` 和过期时间。 |
| `warn` | 硬边界已经存在，但仍有覆盖不足、范围受限或到期整改项；必须绑定 owner、due、允许范围和复审触发条件。 |
| `block` | 缺少工具网关、脱敏 trace、审计、回滚、熔断或安全回归等硬边界；不能靠小流量、人工值守或“模型会听话”降级成 `warn`。 |

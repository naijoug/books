...
如果评审会不是从“填表”开始，而是从一个具体争议开始，可以先回到根目录 `README.md` 的 90 分钟审查材料选择表，判断争议属于工具边界、测试证据、灰度止损、硬阻断风险还是会后交接。然后再用下面这张表进入本清单对应区域：

| 现场问题 | 先填本清单 | 需要带回正文或附录的材料 |
|---|---|---|
| “这个 Agent 到底能动哪些生产资源？” | A.3 工具与权限清单 | 第 7 章工具安全元数据、附录 B 的门禁填写粒度 |
| “安全回归失败能不能先灰度？” | A.5 评估与门禁清单 | 第 8 章失败样本、附录 B 的 `warn` / `block` 边界 |
| “上线后谁看指标、怎么停？” | A.6 监控、止损与回滚清单 | 第 9 章发布报告、附录 C 的会后交接清单 |
| “日志和 trace 会不会暴露敏感信息？” | A.4 数据、日志与 trace 清单 | 第 10 章数据安全与审计日志段落 |
| “今天到底是 pass、warn 还是 block？” | A.7 发布决策模板 | 附录 B 的填写样例和反例、附录 C 的最终结论脚本 |

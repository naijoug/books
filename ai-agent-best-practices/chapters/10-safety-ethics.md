...
| 生产发布 | 面向真实用户和关键业务流程 | 工具风险分级、审批绑定参数、发布安全门禁、事故 runbook、熔断开关 | 自动优化 Prompt、跨系统批量操作、长期记忆自动合并 |
...
3. **补最小安全测试集**：从 prompt injection、越权工具、敏感数据泄露和失控循环各选 2～3 个样例，先形成会阻断发布的安全 Golden Tasks。
...
如果资源有限，优先投入在“服务端权限网关 + 发布安全门禁 + 熔断 runbook”三件事上。它们分别对应事前防止越权、上线前发现退化、事故中降低影响面，比单纯优化 Prompt 更能形成生产级安全护城河。

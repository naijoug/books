...
| TypeScript | `chapters/typescript/` | 6 张：联合类型、条件类型、`infer`、Mapped Type 和深度只读 |
| React | `chapters/react/` | 8 张：Effect、状态、表单、列表 key、ref、自定义 Hook 和性能 |
| Swift | `chapters/swift/` | 8 张：值语义、async/await、基础值、字符串、集合、switch、可选绑定和闭包 |
| Flutter | `chapters/flutter/` | 7 张：状态、列表、导航、测试、布局、输入控件和组件拆分 |
| AI Agent | `chapters/ai-agent/` | 18 张：Agent 边界、工具、工具结果、记忆、反馈判断、反馈修订、反馈池、迭代上限、心跳工作流、交接、接力取舍、短节拍边界、验证总结、未验证项交接、提交状态读回和助手操作系统 |
...
1. 将 TypeScript、React、Swift、Flutter 和 AI Agent 目录继续扩展到接近 Python 的密度。
2. 为每个代码片段补充可运行环境或最小测试。
3. 新增卡片时继续按“问题、要点、示例、坑、检查”的格式写。
4. 发布前统一术语、难度标识和交叉引用。

...
“下一步”不要写成愿望句，要能被下一轮直接复制或执行。先按失败归属选择第一条动作：

| 失败归属 | 第一条动作 | 如果仍失败 |
|---|---|---|
| 本轮改动 | 复跑最小失败命令，确认失败稳定；再只改相关 path | 回滚本轮相关改动或把失败用例固化为最小测试 |
| 启动前 dirty path | 重新记录该 repo 的 `git status --short`，只读判断归属 | 归属和授权明确时继续；否则暂停受影响部分，先推进原任务内独立工作 |
| 环境依赖 | 先运行安装/版本检查命令，保存原始错误 | 不宣称构建通过，交接所需依赖和可重跑命令 |
| 外部服务 | 先做凭据/网络/服务健康检查，不修改业务代码 | 标记为外部阻塞，给出替代 dry-run 或 mock 验证 |
| 信息缺口 | 先读取权威文档、配置或入口文件 | 若仍缺失，列出需要用户决策的问题，停止有副作用动作 |

第一条动作的标准是：范围最小、能验证失败是否稳定、不会扩大未知归属。它应该是一个命令、一个文件读取、一个人工检查路径，或一个明确的“停止并等待授权”条件。

...
- **验证入口速记**：[`ai-agent-sample-pack.md`](ai-agent-sample-pack.md) 顶部的 30 秒导航，先看本轮“当前最大风险”，再决定入口；本表是它的 2 分钟展开版。
...
如果 30 秒内已经能判断当前最大风险，先回到 [`ai-agent-sample-pack.md`](ai-agent-sample-pack.md) 顶部“验证入口速记”复制对应输入；只有在 proof、baseline、wrapper、命令梯和交接模板之间仍然摇摆时，再用下面四问展开入口判断。

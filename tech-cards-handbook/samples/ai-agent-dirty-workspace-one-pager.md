...
- 报告类：最终报告模板中的固定字段不能因为“没有发生”而删除；没有项目提交写 `未提交`，没有变更文件写 `无`，未接管边界为空也写 `无`。
...
字段顺序固定为：`本轮选择 -> 实际推进 -> 变更文件 -> 验证证据 -> 写入 notebook -> 项目提交 -> notebook 提交 -> 未接管边界 -> 下一段接力`。如果某一项没有发生，也保留字段并写“无”或“未提交”，不要删除字段让下一轮猜测。

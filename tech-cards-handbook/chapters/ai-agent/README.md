...
| 4 | 无人值守时选择低风险默认动作，而不是等待澄清 | [`unattended-agent-chooses-default-action.md`](unattended-agent-chooses-default-action.md) |
...
运行控制建议按“失败出口 → 心跳闭环 → 可执行交接 → 验证与报告”的顺序阅读：先给 Agent 设置停止条件，再用心跳维持长期任务节奏；每次交接都写清下一步动作，并用验证、显式未验证项和已提交状态约束最终报告。

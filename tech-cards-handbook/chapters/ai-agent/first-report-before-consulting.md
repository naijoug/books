...
- **输出压缩到 1 页**：包含 `Scope`、`Highest Risk`、`Evidence inspected`、`Next Safe Command Ladder`、`Unverified items` 和 `Continue / Narrow / Stop`。
...
交付前检查：输入是否能在 10 分钟内准备；审查是否只读；报告是否压缩到 1 页；每个关键判断是否有 `Fact`、`Inference`、`Unverified` 或 `Stop` 标签；是否写清 `Next evidence needed`；收尾是否明确 `Continue / Narrow / Stop`。只有 `Continue` 有新证据或明确购买意愿时，才进入咨询、模板化或产品化讨论。

...
- **可复用沉淀要显式决策。** 如果邻近模板已经要求沉淀模板、脚本、检查表或案例卡，而被改模板只把它隐含在检查清单外，要么单独补字段，要么在接力记录里写明为什么暂不补。
...
这一行的关键不是“必须所有模板都长一样”，而是不能让读者误以为检查清单已经覆盖沉淀出口。接管时先二选一：补 A.1 的 `可复用沉淀` 字段，或在回执中说明 A.1 只管任务前风险、不承担复盘沉淀。

### Go / Narrow 决策

- **Go**：如果 A.1 的使用场景已经包含“这次任务结束后要留下模板、脚本、检查表或案例卡”，就把 `可复用沉淀` 作为字段对齐单独补入 A.1，并只提交附录模板字段对齐这一组。
- **Narrow**：如果 A.1 仍定位为“使用 AI 前的权限、输入和风险检查”，就不要强行加沉淀字段；在回执中说明沉淀出口由 A.14 / A.15 或复盘模板承接。
- **Stop**：如果同一 diff 还混有章节正文、标点风格、表格结构或 README 审校记录，先不要提交 A.1；把字段判断留到独立接管窗口。

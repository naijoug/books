...
3. **交付层**：提供 1 页报告、PR 模板、issue template 或 handoff 模板，让读者能复制到真实 repo。
4. **转化层**：用 `Continue / Narrow / Stop` 判断是否继续写内容、做服务、做工具，避免靠点赞误判需求。
...
| 交付层 | 我能拿什么交给团队？ | 1 页报告、PR/issue 模板 |
...
3. 然后使用 `ai-coding-audit-service.md` 与 `ai-coding-audit-mock-report.md`，把方法变成固定范围交付。
4. 最后回到 `ai-programmer-asset-flywheel.md`，记录哪些环节值得变成模板、技能、服务或工具。
...
- **直接跳到卖服务**：没有样本报告、命令梯或模板时，服务承诺会显得空泛。
- **把点赞当需求**：AI 话题容易有泛流量，但真正的需求信号是具体失败场景、可审查样本和复盘意向。
...
3. 审查结束后是否有可交付物，而不只是心得？
4. 是否记录了 `Continue / Narrow / Stop` 信号，决定下一步写作、服务或工具化方向？

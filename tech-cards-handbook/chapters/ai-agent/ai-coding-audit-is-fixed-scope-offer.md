...
- 固定输出：1 页风险报告、5 条优先级建议、下一条安全命令梯、一个可复制 handoff 模板。输出越短，越能证明审查能力可重复。
...
- 把每次人工审查当作产品发现：若同一风险在多个团队重复出现，再沉淀成 checklist、CLI、PR bot 或团队 onboarding 模板。
...
**检查**：这个 offer 是否能在不接生产权限的情况下交付；输入是否少到买家 10 分钟内能准备；输出是否压缩到 1 页；报告是否包含证据、风险、下一条命令、handoff 和 `Continue / Narrow / Stop`；复盘后是否能判断下一步是继续服务、缩小范围，还是停止这个收入实验。

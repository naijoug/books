...
- 最终报告只能引用已提交并读回的状态：先完成项目 repo 提交，再用 `git rev-parse --short HEAD` 和 `git log -1 --pretty=%s` 读回 hash 与标题。
...
- 项目提交：无（如果本轮没有项目 repo 变更，就这样写明原因）
```
...
# notebook repo（当前固定为 summaries），notebook 写完后
...
- 项目 repo 和 notebook repo 都提交了，但最终只写一个 hash，下一轮不知道哪个 hash 对应真正的代码/内容改动。

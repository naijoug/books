...
第 1、9、10、25、36-48 张卡片可以组成一条 Hook 与并发安全阅读线:先理解 effect 只负责同步外部系统,并且必须能在 Strict Mode 的额外 setup → cleanup → setup 中正确恢复;再用 `useDeferredValue` 和 `startTransition` 区分紧急与非紧急更新;遇到复杂交互时,用 `useActionState`、`useFormStatus`、`useOptimistic` 管住提交与乐观视图;最后用 `useId`、hydration 稳定输入、`useSyncExternalStore`、Strict Mode、`useReducer` 和状态机建模检查 SSR 一致性、外部 store 快照、副作用幂等性、复杂状态转移边界和 UI 不可能状态。

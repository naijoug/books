...
代码块验证:在 `books` 仓库根目录运行 `python3 scripts/verify_react_cards.py`,脚本会抽取本章 `ts`/`tsx`/`typescript` 代码块,写入临时 `.tsx` 源文件,再用一次 TypeScript strict 编译和轻量 React 类型 shim 做批量检查。也可以通过统一入口只验证 React：`python3 scripts/verify_all_cards.py --language React`。
...
[`派生状态不要镜像 props`](derived-state-not-mirrored-props.md)、[`URL 状态应该留在 URL`](url-state-belongs-in-url.md)、[`useState 函数式更新`](usestate-functional-update.md)、[`异步状态联合类型`](async-state-discriminated-union.md)、[`Context 拆分状态和动作`](context-split-state-and-actions.md)、[`useReducer`](usereducer-centralizes-complex-state-transitions.md) 和 [`状态机`](state-machine-eliminates-impossible-ui-states.md) 可以组成一条从局部状态到流程状态的建模路径:能由当前 props/state 推导出来的值先留在渲染阶段,不要复制成同步 state;会影响页面位置、刷新复现和分享链接的筛选/排序/分页状态优先归到 URL;简单计数或切换再用 `useState` 的函数式更新避免闭包旧值;异步读取和提交结果用联合类型表达互斥状态;跨层共享时先拆分 Context 的 state 与 actions;当事件分支变多,再把状态转移收拢到 `useReducer`;最后用显式状态机检查步骤流里的合法转移和不可能状态。
...
3. **URL 归属**:搜索词、筛选、排序、分页和 tab 等可刷新/可分享状态是否放在 URL;浏览器前进后退时 UI 与请求 key 是否同步变化。
4. **旧值更新**:依赖前一状态计算新状态时,是否使用函数式更新,避免事件闭包或批处理导致读到旧值。
5. **互斥状态**:加载中、成功、空结果、字段错误、系统错误等是否用联合类型表达;是否避免多个布尔值拼出 impossible state。
6. **事件表完整**:是否列清楚哪些事件能改变状态;每个事件在当前状态下是允许、忽略还是报错,而不是让任意分支随时写状态。
7. **Context 边界**:Context 是否拆分 state 与 actions;高频变化的数据是否没有放进会导致全树重渲染的全局 Provider。
8. **Reducer 纯度**:`useReducer` 是否只计算下一状态;网络请求、日志、导航等副作用是否放在事件处理或 Effect 边界。
9. **状态机合法性**:多步骤流程是否有显式状态机或转移表;取消、返回、重试、超时等边缘事件是否都有合法去向。
10. **异步回包归属**:请求或提交结果回来时,是否校验它仍属于当前状态/版本;过期结果是否不会覆盖用户后续操作。
11. **可测试性**:核心状态转移是否能用输入状态 + 事件 → 输出状态单独测试;复杂 UI 是否不需要靠手点流程才能证明正确。
...
| URL 状态应该留在 URL | [`url-state-belongs-in-url.md`](url-state-belongs-in-url.md) |

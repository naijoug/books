...
代码块验证:在 `books` 仓库根目录运行 `python3 scripts/verify_react_cards.py`,脚本会抽取本章 `ts`/`tsx`/`typescript` 代码块,写入临时 `.tsx` 源文件,再用一次 TypeScript strict 编译和轻量 React 类型 shim 做批量检查。

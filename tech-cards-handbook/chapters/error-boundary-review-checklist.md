...
错误边界审查很容易从“看一个 handler”滑向“顺手重构整个错误系统”。为了让 agent 或 reviewer 的输出可落地，先按下面规则决定本轮只收敛范围，还是可以升级为修复建议。

| 观察到的证据 | 本轮动作 | 不要做 |
|---|---|---|
| 只有模糊需求，例如“帮我看看错误处理” | `NARROW_FIRST`：要求补待审相对路径、入口函数、公开响应契约和失败样例 | 不要假设框架、handler 名或数据库驱动 |
| 有 diff hunk，但看不到调用方动作 | 只评论“分类/恢复动作证据不足”，要求补 service/handler 调用链 | 不要直接断言应该 retry 或 degrade |
| 有调用链和公开响应，但没有测试 | 输出 P1/P2 finding，并把“公开响应脱敏 + cause 保留 + retry/degrade 分支”写成建议测试 | 不要把“看日志确认”当作验证 |
| 有决策表、失败测试和 trace | 可以升级为具体修复建议或小型 patch；每个 patch 只改一个错误流 | 不要同时改错误类型、接口契约、日志格式和重试策略 |

实操上，先把本轮审查范围压到“一条入口 -> service -> adapter 调用链”。如果一个 finding 需要跨第二条调用链才能证明，就把它放入“后续接力”，不要在当前短评里扩大结论。这样做会牺牲一点覆盖面，但能显著减少 agent 编造证据、误判降级语义和混入无关重构的风险。

...
7. **把恢复动作收束到一张表**:如果重试、降级、字段错误、登录跳转和安全错误分散在多个调用方,用 Swift 的 [`swift/swift-error-recovery-decision-table.md`](swift/swift-error-recovery-decision-table.md)、Flutter 的 [`flutter/flutter-error-recovery-decision-table.md`](flutter/flutter-error-recovery-decision-table.md) 或后端语言的决策表卡片把领域错误码、调用方动作和 UI / 响应契约对齐。

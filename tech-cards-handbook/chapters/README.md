...
4. **最后确认恢复动作显式化**：读 Rust 的 [`rust/retry-strategy-explicit-not-implicit-loop.md`](rust/retry-strategy-explicit-not-implicit-loop.md)，把“哪些错误可重试、最多重试几次、如何退避、耗尽后返回什么”从临时 `loop` / 嵌套 `match` 中拆成可测试的策略。
